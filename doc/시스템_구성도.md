...
- **DB 확장 경로**: `dj_database_url` 기반 설정으로 `DATABASE_URL` 환경변수만 지정하면 SQLite → PostgreSQL(RDS)로 무중단 전환 가능하도록 설계됨 (`backend/settings.py:91-107`). 현재는 미적용(SQLite 사용 중).
- **정적 자원 확장 경로**: 현재는 WhiteNoise + Nginx `/static/` alias 조합이며, 트래픽 증가 시 `django-storages` + S3/CloudFront로 전환 가능 (미적용).

> RDS·S3는 아직 적용되지 않았으며, 위 두 항목은 현재 구조에서 설정 변경만으로 확장 가능한 "예정된 확장 경로"로 표기함. 자세한 미구현 현황은 `doc/개발된_LLM_연동_웹_애플리케이션.md`의 [미구현/설계 차이 정리] 참고.

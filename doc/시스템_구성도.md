...
클라이언트 요청이 Nginx → Gunicorn(Django) → (DB / 벡터DB / 랭커) → 외부 LLM API 순으로 처리되고, 응답은 SSE로 스트리밍되어 되돌아온다.
...
6. 상담 이력은 `ChatHistory` 모델을 통해 SQLite(`DATABASE_URL` 미설정 시 기본값)에 저장된다.
...
- **DB 확장 경로**: `dj_database_url` 기반 설정으로 `DATABASE_URL` 환경변수만 지정하면 SQLite → PostgreSQL(RDS)로 무중단 전환 가능하도록 설계됨 (`backend/settings.py:91-107`). 현재는 미적용(SQLite 사용 중), 전환 절차는 `doc/rds-postgres-migration.md` 참고.
...
> RDS와 정적 자원용 S3(django-storages)는 아직 적용되지 않았으며, 설정 변경만으로 확장 가능한 "예정된 확장 경로"로 표기함. 벡터 DB 백업용 S3 동기화는 이미 배포 파이프라인에 적용됨. 자세한 현황은 `doc/개발된_LLM_연동_웹_애플리케이션.md`의 [미구현/설계 차이 정리] 참고.

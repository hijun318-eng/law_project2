...
- 근거: `docker/`, `apps/backend/backend/settings.py`, `apps/backend/engine/config.py`, `doc/manuals/aws-ec2-two-server-deploy.md`, `doc/manuals/runpod-ranker-deploy.md`
...
| **AWS RDS** | Django `ChatHistory` 등 관계형 데이터를 담당하는 관리형 PostgreSQL. `DATABASE_URL` 환경변수(`dj_database_url.parse`)로 접속하며, `DB_CONN_MAX_AGE`(커넥션 풀링)·`DB_SSL_REQUIRE`(SSL 강제)를 함께 설정 (`backend/settings.py:91-107`, `doc/manuals/rds-postgres-migration.md`) |
...
- RDS는 퍼블릭 액세스를 차단하고 Backend EC2 보안그룹에서만 5432 인바운드를 허용하며, `DB_SSL_REQUIRE`로 전송 구간 암호화를 강제할 수 있다 (`backend/settings.py:98`, `doc/manuals/rds-postgres-migration.md`).
...
- **DB 확장 경로 (RDS, 적용됨)**: `dj_database_url` 기반 설정으로 `DATABASE_URL` 환경변수에 RDS 엔드포인트를 지정해 PostgreSQL(AWS RDS)로 무중단 전환 가능하도록 설계됨 (`backend/settings.py:91-107`). 로컬 개발 환경은 `DATABASE_URL` 미설정 시 SQLite로 자동 대체되어 별도 DB 없이도 개발 가능. 커넥션 풀링(`DB_CONN_MAX_AGE`)·SSL 강제(`DB_SSL_REQUIRE`) 옵션 포함, 상세 절차는 `doc/manuals/rds-postgres-migration.md` 참고.

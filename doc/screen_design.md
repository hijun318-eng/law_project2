...
| 로그인 완료 사용자 접근 | SC-3.1 또는 SC-4.1 |
...
사용자가 이메일과 비밀번호를 입력해 서비스에 접근하는 인증 화면이다. 현재 구현은 데모 로그인 방식으로 이메일에 따라 일반 사용자와 관리자를 구분한다.
...
| 비밀번호 | HTML5 `required` |
| 인증 실패 | 현재 데모 구현에서는 비밀번호 불일치 검증 없음. 설계 보완 시 `"이메일 또는 비밀번호가 일치하지 않습니다."` 표시 |
...
| 일반 사용자 로그인 성공 | SC-3.1 AI 노동법 상담 |
...
신규 사용자가 이름, 이메일, 비밀번호를 입력해 일반 사용자 계정으로 서비스를 시작하는 화면이다. 현재 구현은 별도 DB 저장 없이 session에 사용자 정보를 저장한다.
...
| 비밀번호 확인 | HTML5 `required`. 설계 보완 시 비밀번호 일치 검사 필요 |
| 이메일 중복 | 현재 DB 저장 구조가 없어 중복 검사 없음. 설계 보완 시 `"이미 가입된 이메일입니다."` 표시 |

### 연관 화면

| 조건 | 이동 화면 |
|---|---|
| 회원가입 성공 | SC-3.1 AI 노동법 상담 |
| 로그인으로 클릭 | SC-2.1 로그인 |

...
| SC-4.5 | 관리자 | 성능 모니터링 | `/admin-console/?tab=performance` | `admin_console` | `labor/admin.html`, `_admin_performance.html` |
...
사용자의 상담 이력, 계정 정보를 탭으로 확인하는 개인 화면이다.

### UI 요소 명세

| 번호 | 요소명 | 타입 | 설명 |
|---|---|---|---|
| ① | 탭 버튼 | Tabs | 상담 이력, 계정 정보 전환 |
...
| 항목 | 내용 |
|---|---|
| 이력 없음 | `"상담 이력이 없습니다."` 표시 |
| 비로그인 접근 | `/login/`으로 redirect |
| 이름/이메일/가입일 값 누락 | 각각 "-"로 대체 표시 |
| 상담 횟수 값 누락 | "0회"로 대체 표시 |
...
관리자가 서비스 운영 현황을 통계 카드, 일별 현황, 카테고리 분포, 카테고리별 증감 추이, 사용자 현황, 낮은 평가 답변으로 확인하는 화면이다.
...
| ④ | 일별 현황 | Chart | 질문 수/사용자 수 추이 표시 |
| ⑤ | 카테고리 분포 | Meter List | 카테고리별 비율 표시 |
| ⑥ | 카테고리별 증감 추이 | Chart/List | 전주 대비 카테고리별 질문 비중 변화 표시 |
| ⑦ | 사용자 현황 | Stat + Button | 전체/활성/비활성 사용자 수 표시 |
| ⑧ | 낮은 평가 답변 | List | 카테고리별 낮은 평가 비율 표시 |
...
| 사용자 관리 버튼 클릭 | SC-4.2 사용자 관리로 이동 |
| 전체 보기 버튼(낮은 평가 답변) 클릭 | SC-4.3 피드백 관리 탭으로 이동 |

### 유효성 검사 / 예외 처리

| 항목 | 내용 |
|---|---|
| 통계 카드 / 일별 현황 / 카테고리별 증감 추이 / 낮은 평가 답변 데이터 없음 | 각 영역에 "표시할 ~ 데이터가 없습니다." 형태의 빈 상태 문구 표시 |
| 카테고리 분포 데이터 없음 | 도넛 차트를 빈 회색 원으로 표시, 범례에 "표시할 카테고리 데이터가 없습니다." 표시 |
| 카테고리 분포 값 누락/음수/NaN | 해당 구간 값을 0으로 처리 |
| 카테고리 분포 합계 100% 미만 | 남은 구간을 빈 색상으로 채워 원이 어색하게 잘리지 않도록 처리 |
| 도넛 차트 렌더링 오류 | `try/catch`로 감싸 오류가 나도 대시보드의 다른 스크립트 초기화에 영향 없음 |
...
| 전체 보기(낮은 평가 답변) 클릭 | SC-4.3 피드백 관리 |
| 로그아웃 클릭 | SC-1.1 랜딩 |
...
| 이벤트 | 처리 흐름 |
|---|---|
| 계정 정지/해제 클릭 | 행/사용자 id 확인 → 처리 중 버튼 비활성화(중복 클릭 방지) → API 호출 성공 시에만 `data-status`, 배지, 버튼 텍스트 변경 |
...
| 사용자 id 누락/비정상 | API 호출 없이 `"사용자 정보를 확인할 수 없습니다. 새로고침 후 다시 시도해주세요."` 표시 |
| 상태 변경 저장 실패 | UI를 변경하지 않고(자동 롤백) `"계정 상태 변경에 실패했습니다. 잠시 후 다시 시도해주세요."` 표시 |
| 처리 중 중복 클릭 | 요청 처리 중 버튼 비활성화로 중복 요청 방지 |
...
| Model / 데이터 | `dashboard.feedback_context()`, `feedback_api`(좋아요/싫어요 저장 API) 응답 반영 결과 |
...
AI 노동법 상담에 대한 사용자 피드백을 통계 형태로 확인하고, 카테고리별 만족도를 분석하여 개선이 필요한 영역을 우선적으로 관리하는 화면이다.
...
| ② | 카테고리별 만족도 순위 | Stat Card | 카테고리별 평균 만족도를 낮은 순서으로 표시 |
| ③ | 개선 필요 탭 | Tab | 전체 카테고리 / 개서 필요 카테고리 필터 |
| ④ | 카테고리 피드백 그룹 | Acordion | 카테고리별 만족도 및 통계 표시 |
| ⑤ | 개선 필요 배지 | Badge |평균 만족도 60% 미만 카테고리에 표시 |
| ⑥ | 일별 좋아요/싫어요 차트 | Bar Chart |날짜별 좋아요·싫어요 건수 시각화 |
| ⑦ | 카테고리 통계 | Summary |평균 만족도와 낮은 평가 건수 표시 |
| ⑧ | 펼침 아이콘 | Chevron |카테고리 상세 통계 열기/닫기 |

### 기능 / 이벤트 설명

| 이벤트 | 처리 흐름 |
|---|---|
| 전체 카테고리 탭 클릭 | 모든 카테고리 그룹 표시 |
| 개선 필요 탭 클릭 | data-low="true"인 카테고리만 표시 |
| 카테고리 헤더 클릭 | 해당 카테고리의 일별 통계 차트 펼침/접기 |
| 펼침 상태 변경 | Chevron 아이콘 회전 |
| 화면 진입 | 만족도 통계 및 차트 생성 |

### 유효성 검사 / 예외 처리

| 항목 | 내용 |
|---|---|
| 피드백 데이터 없음 | 평균 만족도 0%, 목록 비어 있음 |
| 좋아요 값 누락 | item.get("liked", False)를 사용하여 False 처리 |
| 카테고리 누락 | "미분류" 카테고리로 분류 |
| 날짜 데이터 없음 | 차트 생성 생략 |
| 개선 대상 없음 | "개선 필요 (0)"으로 표시하며 모든 카테고리 정상 조회 |
| message_id 형식 오류(feedback_api) | 문자열로 전달된 숫자도 허용하도록 정수 변환 후 검증. 변환 실패 시 `"message_id가 필요합니다."` 400 응답 |
| action 값 오류(feedback_api) | `like`/`dislike`가 아니면 `"올바르지 않은 action입니다."` 400 응답 |
| 대상 메시지 없음(feedback_api) | `"메시지를 찾을 수 없습니다."` 404 응답 |
| 요청 본문 파싱 실패(feedback_api) | `"요청 본문을 해석할 수 없습니다."` 400 응답 |
| 피드백 저장 실패(feedback_api) | `"피드백 저장 중 오류가 발생했습니다."` 500 응답 |
...
| 이벤트 | 처리 흐름 |
|---|---|
| 템플릿 선택 | `action=get` 호출 → 성공 시 제목/본문/placeholders/history 갱신, 조회 중 재선택 방지 |
| 저장 클릭 | 본문이 비어있으면 오류 표시 후 중단 → `action=validate` 호출(처리 중 버튼 비활성화) → 오류 있으면 오류 박스 표시, 없으면 확인 모달 표시 |
| 즉시 적용 클릭 | `action=save` 호출(처리 중 버튼 비활성화) → 성공 시 저장 상태 표시 후 템플릿 재조회 |
| 롤백 클릭 | `action=rollback` 호출(처리 중 버튼 비활성화) → 성공 시 선택 템플릿 재조회 |

### 유효성 검사 / 예외 처리

| 항목 | 내용 |
|---|---|
| 저장 내용 없음 | 검증 요청 없이 즉시 `"프롬프트 내용을 입력해주세요."` 표시 |
| 프롬프트 검증 오류 | `validate` 응답의 `errors`를 `promptErrors`에 표시 |
| 조회/검증/저장/롤백 API 실패(네트워크·서버 오류) | 각 동작에 맞는 오류 메시지(예: `"저장 중 오류가 발생했습니다. 잠시 후 다시 시도해주세요."`)를 오류 박스에 표시하고 콘솔에 상세 로그 기록 |
| 처리 중 중복 클릭 | 요청 처리 중에는 해당 버튼을 비활성화하여 중복 저장/롤백 방지 |
...
![SC-4.5 성능 모니터링](./screenshots/SC-4.5_admin_performance.png)
 
### 화면 설명
 
LangGraph 노드별 처리 시간, LLM/임베딩 API 호출량, 예상 비용, 병목 노드를 확인하는 운영 모니터링 화면이다. 일/주/월 단위는 단순 표시 단위 변경이 아니라 각각 롤링 윈도우(최근 30일 / 최근 12주 / 최근 12개월)로 조회 범위를 제한하여, 서비스 운영 기간이 길어져도 차트 길이가 무한정 늘어나지 않도록 한다.
 
### UI 요소 명세
 
| 번호 | 요소명 | 타입 | 설명 |
|---|---|---|---|
| ① | 노드 처리 시간 카드 | Stat Card | 노드명, 호출 수, 평균/최대 처리 시간. 데이터 없으면 "표시할 노드 데이터가 없습니다." 표시 |
| ② | 기간 선택 버튼 | Segmented Button | 일(최근 30일)/주(최근 12주)/월(최근 12개월) 전환, 기본값 '일' |
| ③ | API 사용량 카드 | Stat Card | 총 LLM 호출, 총 토큰 사용량, 예상 총 비용 |
| ④ | 조회 범위 라벨 | Text | API 호출 차트 옆에 현재 조회 범위(예: "최근 30일") 표시 |
| ⑤ | API 호출 차트 | Bar Chart | 선택 기간의 일/주/월별 LLM·임베딩 호출 수 시각화. 데이터 없으면 "표시할 호출 데이터가 없습니다." 표시 |
| ⑥ | 병목 노드 TOP | List/Bar | 노드별 부하 비율 표시. 데이터 없으면 "표시할 병목 데이터가 없습니다." 표시 |
| ⑦ | 상태 배너 | Banner | 로딩 중 / 조회 실패 메시지와 재시도 버튼 표시 |
 
### 기능 / 이벤트 설명
 
| 이벤트 | 처리 흐름 |
|---|---|
| 화면 진입 | '일' 기준(최근 30일)으로 자동 조회 후 초기 렌더링 |
| 기간 버튼 클릭 | active class 변경 → 해당 기간의 롤링 윈도우로 API 재조회 → 노드 카드, API 사용량 카드, 조회 범위 라벨, 차트, 병목 노드 TOP 전체를 응답 데이터로 다시 그림 |
| 조회 중 | 상태 배너에 "데이터를 불러오는 중입니다..." 표시 |
| 재시도 버튼 클릭 | 마지막으로 선택한 기간으로 재조회 |
| 응답 지연 중 다른 기간 클릭 | 늦게 도착한 이전 요청의 응답은 화면에 반영하지 않고 폐기 |
 
### 유효성 검사 / 예외 처리
 
| 항목 | 내용 |
|---|---|
| API 호출 실패 (네트워크 오류, 4xx/5xx 응답) | 상태 배너에 `"성능 데이터를 불러오지 못했습니다. 잠시 후 다시 시도해주세요."` 표시 + 재시도 버튼 노출, 콘솔에 상세 오류 로그 기록 |
| 노드/차트/병목 데이터 없음 | 각 영역에 개별 빈 상태 문구 표시 |
| 통계 값 누락 | `total_calls`, `total_tokens`는 0, `total_cost`는 "0"으로 대체 표시 |
| 서버 응답 노드 개수 변경 | 기존 카드를 부분 갱신하지 않고 응답 기준으로 전체를 다시 렌더링하여 개수 불일치로 인한 표시 오류 방지 |
| 조회 범위 확장(장기 데이터) | 백엔드에서 기간별로 최근 30일/12주/12개월로 조회 범위를 제한·집계하여 반환. 그 이상의 과거 데이터는 이번 화면 범위에서 제외 |
...
| `/api/admin/performance/` | GET | SC-4.5 | `period` (query) | `{ langgraph_nodes, llm_usage, total_calls, total_tokens, total_cost }` | 기간 없는 경우 day 기본값 |
